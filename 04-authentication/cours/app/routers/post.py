...
async def delete_post_by_id(post_id: str, db: Session = Depends(database.get_db), user_id: str = Depends(get_user_id)):
    try:
        return posts_service.delete_post_by_user(post_id=post_id, db=db, user_id=user_id)
    except PostNotFound:
